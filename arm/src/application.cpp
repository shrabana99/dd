...
private:
  ...
  void getModelStates(const gazebo_msgs::ModelStates &msgs)
  {
    ...
    for (int i = 0; i < msgs.pose.size(); i++)
    {
      ...
      msgs.name[i].find("target1", pos) != std::string::npos ) 
      {
        detected_model = true;
      }
    }
  ...
  void generate_object()
  {
    ...
    for (int i = 0; i < model.pose.size(); i++)
    {
      ...
      model.name[i].find("target1", pos) != std::string::npos )
      {
        ...
        std::cout << "Collision object name: " << model.name[i] << " " << model.pose.size() << std::endl;
  ...
  void set_targets(std::string topic)
  {
    ...
    for (int j = 0; j < model.pose.size(); j++)
    {
      ...
      if (model.name[j].find("target", pos) != std::string::npos ||
        model.name[j].find("target1", pos) != std::string::npos )
      {
        for (int i = 0; i < pose_array.poses.size(); i++)
        {
          ...
          {
            std::cout << "Target name: xxxxxxxxxxxxxxxxxxxxxxxxx" << name << std::endl;
